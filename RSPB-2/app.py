...
with st.sidebar:
    ...
    if voice_enabled:
        st.success("✅ Voice functionality enabled")
        ...
        st.error("❌ Voice functionality disabled")
...
if st.session_state.mode == "Activity Guide":
    ...
    def start_voice_input():
        ...
        if not voice_enabled or not st.session_state.voice_input_enabled:
            st.toast("Voice input is not available or disabled", icon="🎤")
            return
        
        st.session_state.is_listening = True
        st.toast("Listening... Speak your task now!", icon="🎤")
        ...
        if spoken_text:
            ...
            st.toast(f"Heard: '{spoken_text}'", icon="✅")
            # Automatically start the task with the spoken text
            start_task_with_text(spoken_text)
        else:
            st.toast("No speech detected or recognition failed", icon="❌")
    ...
    with col1:
        ...
        if voice_enabled and st.session_state.voice_input_enabled:
            if st.button("🎤 Speak Task", disabled=st.session_state.is_listening):
                start_voice_input()
        else:
            st.button("🎤 Speak Task", disabled=True, help="Voice input not available")
